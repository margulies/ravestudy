% Statistical comparison of agreebement / reliability of different pipelines

statfile = '/SCR/ellamil/ravestudy_stats/pilot/disco_reliability_anova.mat';
corrfile = '/SCR/ellamil/ravestudy_stats/pilot/disco_reliability.mat';
...
for i = 1:length(pipeline)
    ...
    if ~isempty(strfind(paramname{i}{4},'coif'))
        paramvalue(i,2) = 1;        
    elseif ~isempty(strfind(paramname{i}{4},'db'))
        paramvalue(i,2) = 2;
    elseif ~isempty(strfind(paramname{i}{4},'sym'))
        paramvalue(i,2) = 3;
    end
    
    % Wavelet decomposition: Vanishing moments
    if ~isempty(strfind(paramname{i}{4},'1')) || ~isempty(strfind(paramname{i}{3},'2'))
        paramvalue(i,3) = 1;
    elseif ~isempty(strfind(paramname{i}{4},'3')) || ~isempty(strfind(paramname{i}{3},'4'))
        paramvalue(i,3) = 2;
    elseif ~isempty(strfind(paramname{i}{4},'5')) || ~isempty(strfind(paramname{i}{3},'6'))
        paramvalue(i,3) = 3;
    end
    
    % Data downsampling
    if strcmp(paramname{i}{5},'decimate')
        paramvalue(i,4) = 1;
    elseif strcmp(paramname{i}{5},'mean')
        paramvalue(i,4) = 2;
    end
    
    % Time interpolation
    if strcmp(paramname{i}{6},'cubic')
        paramvalue(i,5) = 1;
    elseif strcmp(paramname{i}{6},'linear')
        paramvalue(i,5) = 2;
    elseif strcmp(paramname{i}{6},'nearest')
        paramvalue(i,5) = 3;
    end
    
    % Axes combination
    if strcmp(paramname{i}{7},'vector')
        paramvalue(i,6) = 1;
    elseif strcmp(paramname{i}{7},'xalign')
        paramvalue(i,6) = 2;
    elseif strcmp(paramname{i}{7},'yalign')
        paramvalue(i,6) = 3;
    elseif strcmp(paramname{i}{7},'zalign')
        paramvalue(i,6) = 4;
    end    
